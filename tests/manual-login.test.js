...
test("login targets the submit button and allows simultaneous active tabs", () => {
  assert.match(
    appSource,
  ...
  assert.doesNotMatch(appSource, /navigator\.locks\.request/);
  assert.doesNotMatch(appSource, /Passly เปิดใช้งานอยู่ในแท็บอื่น/);
